...
export function InventoryLayout({
        ...
        {count !== undefined && count > 0 && (
          <span
            className={`text-[10px] font-extrabold px-2 py-0.5 rounded-full ${
              isActive ? 'bg-white text-[var(--ink)]' : 'bg-rose-500 text-white'
            }`}
          >
            {count}
          </span>
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row text-[var(--ink)]">
      ...
      <aside
        className={`fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-white border-r border-[#24252c]/[0.08] text-[var(--ink)] p-5 flex flex-col justify-between transition-transform duration-300 ${
          mobileSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
        }`}
      >
        <div>
          <div className="pb-6 mb-6 border-b border-[#24252c]/[0.08] flex items-center justify-between">
