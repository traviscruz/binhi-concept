...
export default function InventoryProfilePage({ go }: { go: (p: Page) => void }) {
  const [userId, setUserId] = useState<string | null>(null);
  const [firstName, setFirstName] = useState('Ely');
  const [lastName, setLastName] = useState('Buendia');
  const [email, setEmail] = useState('ely.buendia@binhi.ph');
  ...
  // Load current Inventory Manager user profile from Supabase
  useEffect(() => {
    async function loadInventoryProfile() {
      try {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `inv_${userId || 'user'}_${Date.now()}.${fileExt}`;
      ...
      <div className="border-b border-[#24252c]/[0.06] pb-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[var(--ink)] mt-2">
            Inventory Manager Profile & Security
          </h1>
          <p className="text-xs text-[#24252c]/60 mt-1">
            Manage your inventory manager profile details, avatar, and security credentials.
          </p>
        </div>
      </div>
