...
export default function AdminProfilePage({ go }: { go: (p: Page) => void }) {
  ...
  // Load current Admin user profile from Supabase
  useEffect(() => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `admin_${userId || 'user'}_${Date.now()}.${fileExt}`;
      ...
      setUploadingAvatar(false);
      setProfileSuccessMsg('Admin avatar updated successfully!');
      setTimeout(() => setProfileSuccessMsg(''), 4000);
    } catch (err: any) {
      setAvatarError(err?.message || 'Failed to upload admin avatar.');
      setUploadingAvatar(false);
    }
  };
  ...
  const handleRemoveAvatar = async () => {
    ...
    try {
      ...
      setUploadingAvatar(false);
      setProfileSuccessMsg('Admin avatar removed successfully.');
      setTimeout(() => setProfileSuccessMsg(''), 4000);
    } catch (err: any) {
  ...
  const handleConfirmPasswordChangeWithOtp = async () => {
    ...
    try {
      ...
      setConfirmPassword('');
      setPasswordOtpToken('');
      setPasswordSuccessMsg('Your Admin password has been changed successfully!');
  ...
  return (
    <div className="space-y-8 w-full max-w-5xl mx-auto animate-blur-in">
      ...
      <div className="border-b border-[#24252c]/[0.06] pb-4">
        <div>
          <MonoBadge icon={IconShield}>Admin Account Settings</MonoBadge>
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-[var(--ink)] mt-2">
            Admin Profile & Security
          </h1>
          <p className="text-xs text-[#24252c]/60 mt-1">
            Manage your administrator profile details, avatar, and security credentials.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-[#24252c]/[0.08] shadow-sm">
        <div className="flex items-center gap-2 mb-6 pb-3 border-b border-[#24252c]/[0.06]">
          ...
          <div>
            <h2 className="text-lg font-bold text-[var(--ink)]">Administrator Profile</h2>
      ...
      <div className="bg-white rounded-[2rem] p-6 md:p-8 border border-[#24252c]/[0.08] shadow-sm">
        <div className="flex items-center gap-2 mb-6 pb-3 border-b border-[#24252c]/[0.06]">
          ...
          <div>
            ...
            <p className="text-xs text-[#24252c]/50">Update your Admin password with 6-digit email OTP verification.</p>
          </div>
        </div>
